...
class DataProcessor:
    def __init__(
        ...
        self.data, self.feature_list, self.temporal_data, self.spatial_data = self.load_data(
    ...
    def create_autoregressive_sequences(self, sequence_length=INPUT_SIZE + FH):
        ...
        if self.temporal_encoding:
            ...
            for i in range(self.samples - sequence_length + 1):
                time_sequences[i] = self.temporal_data[i + INPUT_SIZE] # Get time for the last timestamp for x
    ...
    def load_data(self, path=DATA_PATH, spatial_encoding=False, temporal_encoding=False):
        ...
        if spatial_encoding:
            ...
            spatial_data = spatial_data.reshape((1, len(latitudes)*len(longitudes), self.num_spatial_constants))
        else:
            spatial_data = None
