    ...
    def sequence_objective(self, trial, max_sequence_length=15):
        try:
            ...
            if(self.baseline_type == 'simple-linear'):
                    linearreg = SimpleLinearRegressor(X.shape, self.fh, self.feature_list, regressor_type=self.sequence_regressor, alpha=self.sequence_alpha)
                    ...
                    linearreg = LinearRegressor(X.shape, self.fh, self.feature_list, regressor_type=self.sequence_regressor, alpha=self.sequence_alpha)
                    linearreg.train(X_train, y_train, normalize=True)
                    y_hat = linearreg.predict_(X_test, y_test)
                    rmse_values = linearreg.get_rmse(y_hat, y_test, normalize=True)
                    mean_rmse = np.mean(rmse_values)
            ...
            print('Exception occurred: Invalid Baseline, choose between \'linear\' , \'simple-linear\' and  \'lgbm\'')
    ...
    def objective(self, trial):
        try:
            ...
            if(self.baseline_type == 'simple-linear'):
                ...
                linearreg = SimpleLinearRegressor(X.shape, self.fh, self.feature_list, regressor_type=regressor_type, alpha=alpha)
                linearreg.train(X_train, y_train, normalize=True)
                y_hat = linearreg.predict_(X_test, y_test)
                rmse_values = linearreg.get_rmse(y_hat, y_test, normalize=True)
                mean_rmse = np.mean(rmse_values)
                ...
                alpha = trial.suggest_float('alpha', 0.1, self.max_alpha, log=True)
                regressor_type = trial.suggest_categorical('regressor_type', self.regressors)
                linearreg = LinearRegressor(X.shape, self.fh, self.feature_list, regressor_type=regressor_type, alpha=alpha)
                linearreg.train(X_train, y_train, normalize=True)
                y_hat = linearreg.predict_(X_test, y_test)
                rmse_values = linearreg.get_rmse(y_hat, y_test, normalize=True)
                mean_rmse = np.mean(rmse_values)
                ...
                regressor = LightGBMRegressor(X.shape, self.fh, self.feature_list, **params)
                regressor.train(X_train, y_train)
                y_hat = regressor.predict_(X_test, y_test)
                rmse_values = regressor.get_rmse(y_hat, y_test, normalize=True)
                mean_rmse = np.mean(rmse_values)
