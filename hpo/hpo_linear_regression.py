    ...
    try:
        ...
        if baseline_type == "simple-linear":
            linearreg = SimpleLinearRegressor(
                X.shape, fh, feature_list, regressor_type=regressor_type, alpha=alpha
            )
        ...
        print(
            "Exception occurred: Invalid Baseline, choose between 'linear' and 'simple-linear'"
        )
...
def run_hpo(data_path, baseline_type, n_trials, use_neighbours, file_name):
    ...
    best_s, best_regressor_type, best_alpha = run_study(
        baseline_type, n_trials, data, feature_list
    )
